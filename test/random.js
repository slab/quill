...
const getRandomChange = base => {
    ...
    const dimension = {
        ...
        columns: new Delta(base.ops[0].insert['table-embed'].columns || []).length(),
    };
    ['rows', 'columns'].forEach(field => {
        ...
        switch (action) {
            case 'insert':
                ...
                delta.push(attachAttributes({ insert: { id: getRandomRowColumnId() } }));
                break;
            case 'delete':
                if (baseLength >= 1) {
...
const getRandomBase = () => {
    ...
    const table = {};
    if (rowCount)
        table.rows = getRandomRowColumnInsert(rowCount);
    if (columnCount)
        table.columns = getRandomRowColumnInsert(columnCount);
    if (cellCount) {
        const cells = {};
...
const runTestCase = () => {
    ...
    expect(change.compose(change.transform(anotherChange, true))).toEqual(anotherChange.compose(anotherChange.transform(change)));
};
